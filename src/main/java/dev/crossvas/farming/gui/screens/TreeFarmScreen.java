...
public class TreeFarmScreen extends AbstractContainerScreen<TreeFarmMenu> {
    ...
    private static final ResourceLocation TEXTURE =
            new ResourceLocation(CrossFarming.ID,"textures/gui/tree_farm.png");

    public TreeFarmScreen(TreeFarmMenu menu, Inventory inv, Component component) {
    ...
    public void renderEnergyInfo(PoseStack transform, int xPos, int yPos, int mXPos, int mYPos) {
        ...
        if (Helpers.isMouseOver(mXPos, mYPos, pX, pY, 16)) {
            renderTooltip(transform, Component.literal(menu.data.get(0) + "/" + menu.TREE_FARM_TILE.getEnergyStorage().getMaxEnergyStored() + " RF"),
                    mXPos, mYPos);
        }
    }
}
