...
public class InfernalFarmScreen extends AbstractContainerScreen<InfernalFarmMenu> {

    private static final ResourceLocation TEXTURE =
            new ResourceLocation(CrossFarming.ID,"textures/gui/infernal_farm.png");

    public InfernalFarmScreen(InfernalFarmMenu menu, Inventory inv, Component component) {
    ...
    public void renderEnergyInfo(PoseStack transform, int xPos, int yPos, int mXPos, int mYPos) {
        ...
        if (Helpers.isMouseOver(mXPos, mYPos, pX, pY, 16)) {
            renderTooltip(transform, Component.literal(menu.data.get(0) + "/" + menu.INFERNAL_FARM_TILE.getEnergyStorage().getMaxEnergyStored() + " RF"),
                    mXPos, mYPos);
        }
    }
}
