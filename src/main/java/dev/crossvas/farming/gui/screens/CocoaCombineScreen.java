...
public class CocoaCombineScreen extends AbstractContainerScreen<CocoaCombineMenu> {

    private static final ResourceLocation TEXTURE =
            new ResourceLocation(CrossFarming.ID,"textures/gui/combine.png");

    public CocoaCombineScreen(CocoaCombineMenu menu, Inventory inv, Component component) {
    ...
    public void renderEnergyInfo(PoseStack transform, int xPos, int yPos, int mXPos, int mYPos) {
        ...
        if (Helpers.isMouseOver(mXPos, mYPos, pX, pY, 16)) {
            renderTooltip(transform, Component.literal(menu.data.get(0) + "/" + menu.COCOA_COMBINE_TILE.getEnergyStorage().getMaxEnergyStored() + " RF"),
                    mXPos, mYPos);
        }
    }
}
